...
const COST_DATA = {
  ...
  roof: {
    label: 'Roof', icon: '🏠',
    base: { low: 800, mid: 3500, high: 9000 },
    perSqft: { low: 1.2, mid: 2.8, high: 6.0 },
    breakdown: { labor: 60, materials: 35, fixtures: 0, permits: 5 },
    tip: 'Asphalt shingles are the most cost-effective roofing material, lasting 20–30 years. Note: enter your total roof area, not room size.'
  },
  garage: {
    label: 'Garage', icon: '🚗',
    base: { low: 1500, mid: 5000, high: 12000 },
...
function selectRoom(key) {
  ...
  const hint = document.querySelector('#sqft-input ~ .hint');
  if (hint) hint.textContent = key === 'roof'
    ? 'Enter total roof area (most homes: 1,500–3,000 sq ft)'
    : 'Measure length × width of the room';
  const lbl = document.querySelector('label[for="sqft-input"]');
  if (lbl) lbl.textContent = key === 'roof' ? 'Roof Area (square feet)' : 'Room Size (square feet)';
}
...
function calculateAndShow() {
  ...
  // Calculate mid (average) first, then derive low/high as ±35%/+45% of mid
  const baseMid  = (room.base.mid  + room.perSqft.mid  * sqft) * stateMult * qualMult * scopeMult;
  const baseLow  = baseMid * 0.65;   // average minus ~35%
  const baseHigh = baseMid * 1.45;   // average plus ~45%
